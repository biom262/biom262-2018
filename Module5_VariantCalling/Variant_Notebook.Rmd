...
```{r}
recurrent<-myjoinVar %>% filter(type!="synonymous SNV" & type!="nonframeshift deletion" & type!="nonframeshift substitution") %>% select(gene,SAMPLE) %>% unique() %>% group_by(gene) %>% summarize(Nsample=length(SAMPLE))
```
